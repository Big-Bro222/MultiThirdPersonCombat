...
public static partial class PlayerLoopUtility
{
    ...
    public static bool AddPlayerLoopSystem(Type playerLoopSystemType, Type targetLoopSystemType,
        PlayerLoopSystem.UpdateFunction updateFunction, int position = -1)
    {
        ...
        for (int i = 0, subSystemCount = loopSystem.subSystemList.Length; i < subSystemCount; ++i)
        {
            ...
            if (subSystem.type == targetLoopSystemType)
            {
                ...
                if (position >= 0)
                {
                    ...
                    Debug.LogWarningFormat("Added Player Loop System: {0} to: {1} position: {2}/{3} method: {4}",
                        playerLoopSystemType.Name, subSystem.type.Name, position, subSubSystems.Count - 1,
                        updateFunction.Method.Name);
                }
                else
                {
                    subSubSystems.Add(targetSystem);
                    Debug.LogWarningFormat("Added Player Loop System: {0} to: {1} position: {2}/{2} method: {3}",
                        playerLoopSystemType.Name, subSystem.type.Name, subSubSystems.Count - 1,
                        updateFunction.Method.Name);
                }
        ...
        Debug.LogErrorFormat("Failed to add Player Loop System: {0} to: {1}", playerLoopSystemType.Name,
            targetLoopSystemType.Name);

        return false;
    }

    public static bool AddPlayerLoopSystem(Type playerLoopSystemType, Type targetSubSystemType,
        PlayerLoopSystem.UpdateFunction updateFunctionBefore, PlayerLoopSystem.UpdateFunction updateFunctionAfter)
    {
        if (playerLoopSystemType == null || targetSubSystemType == null ||
            (updateFunctionBefore == null && updateFunctionAfter == null))
            return false;
            ...
            for (int j = 0, subSubSystemCount = subSystem.subSystemList.Length; j < subSubSystemCount; ++j)
            {
                ...
                if (subSubSystem.type == targetSubSystemType)
                {
                    ...
                    if (updateFunctionBefore != null)
                    {
                        ...
                        Debug.LogWarningFormat("Added Player Loop System: {0} to: {1} before: {2} method: {3}",
                            playerLoopSystemType.Name, subSystem.type.Name, subSubSystem.type.Name,
                            updateFunctionBefore.Method.Name);
                    ...
                    if (updateFunctionAfter != null)
                    {
                        ...
                        Debug.LogWarningFormat("Added Player Loop System: {0} to: {1} after: {2} method: {3}",
                            playerLoopSystemType.Name, subSystem.type.Name, subSubSystem.type.Name,
                            updateFunctionAfter.Method.Name);
                    }
        ...
        for (int i = 0, subSystemCount = loopSystem.subSystemList.Length; i < subSystemCount; ++i)
        {
            ...
            for (int j = subSubSystems.Count - 1; j >= 0; --j)
            {
                if (subSubSystems[j].type == playerLoopSystemType)
                {
                    ...
                    Debug.LogWarningFormat("Removed Loop System: {0} from: {1}", playerLoopSystemType.Name,
                        subSystem.type.Name);
                }
            }
