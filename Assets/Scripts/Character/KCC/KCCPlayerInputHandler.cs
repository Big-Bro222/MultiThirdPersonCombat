...
namespace BigBro
{
    ...
    public class KCCPlayerInputHandler : NetworkBehaviour, IBeforeUpdate, IBeforeAllTicks, IBeforeTick
    {
        ...
        /// </summary>
        public GameplayInput FixedInput
        {
            get
            {
        ...
        /// </summary>
        public GameplayInput RenderInput
        {
            get
            {
        ...
        /// </summary>
        public GameplayInput CachedInput
        {
            get
            {
                CheckRenderAccess(false);
        ...
        /// </summary>
        public float LookResponsivity
        {
            get { return _lookResponsivity; }
            set { _lookResponsivity = value; }
        }
        ...
        [Range(0.0f, 0.1f)]
        [Tooltip(
            "Look rotation delta for a render frame is calculated as average from all frames within responsivity time.")]
        ...
        [Range(0.0f, 5.0f)]
        [Tooltip(
            "Maximum extension of ignore input window if a simulation instability is detected after resuming app.")]
        ...
        void IBeforeUpdate.BeforeUpdate()
        {
            ...
            //TODO: Implement the logic of Lock cursor
            // if (Application.isMobilePlatform == false || Application.isEditor == true)
            // {
            //     Keyboard keyboard = Keyboard.current;
            //
            //     // Cursor lock processing.
            //     if (keyboard != null && (keyboard.enterKey.wasPressedThisFrame == true ||
                ...
                else
                {
                    ...
                    else
                    {
                        if (Runner.Simulation.Stage == SimulationStages.Forward)
                        {
                            ...
                            {
                                Debug.LogWarning(
                                    $"Missing input for {Object.InputAuthority} {Runner.Simulation.Tick}. In Row: {_missingInputsInRow} Total: {_missingInputsTotal} Repeating Last Known Input: {_repeatTime <= _maxRepeatTime}",
                                    gameObject);
                            }
                        }
        ...
        private void ActivateIgnoreInputWindow()
        {
            ...
            Debug.LogWarning($"Activating {_ignoreTime:F3}s input ignore window with {_ignoreExtension:F3}s extension.",
                gameObject);
        }
        ...
        private void TryExtendIgnoreInputWindow(string reason)
        {
            ...
            {
                Debug.LogWarning(
                    $"Detected simulation instability after resuming app. Extending ignore input window by {consumeExtension:F3}s to {_ignoreTime:F3}s with remaining extension {_ignoreExtension:F3}s. Reason:{reason}",
                    gameObject);
            }
        }
        ...
        private void ProcessStandaloneInput()
        {
            ...
            Vector2 mouseDelta = _characterInputProvider.look;
        ...
        private void CleanLookRotationDeltaHistory(Vector2 lookRotationDelta)
        {
            ...
            if (lookRotationDelta.x < 0.0f)
            {
                for (int i = 0; i < count; ++i)
                {
                    if (_frameRecords[i].LookRotationDelta.x > 0.0f)
                    {
                        _frameRecords[i].LookRotationDelta.x = 0.0f;
                    }
                }
            }
            ...
            if (lookRotationDelta.y < 0.0f)
            {
                for (int i = 0; i < count; ++i)
                {
                    if (_frameRecords[i].LookRotationDelta.y > 0.0f)
                    {
                        _frameRecords[i].LookRotationDelta.y = 0.0f;
                    }
                }
            }
